...
export function ViewUserModal({ isOpen, onClose, user, sensors = [] }) {
  ...
  if (
    ...
    sensorName =
      sensors.find((s) => s.sensor_id.toString() === user.device_id.toString())
        ?.sensor_name || `Sensor ID: ${user.device_id}`;
  }

  // Format address display
  const formattedAddress = user.street_address
    ? `${user.street_address}${
        user.phase_number ? ` Phase ${user.phase_number}` : ""
      }`
